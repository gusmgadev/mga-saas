"use client";

import { useState } from "react";
import { useForm } from "react-hook-form";
import { motion } from "framer-motion";
import { BRAND, CONTACT, SECTION_COLORS } from "@/lib/constants";

type ContactFormData = {
  name: string;
  email: string;
  phone: string;
  message: string;
};

export function ContactForm() {
  const [submitted, setSubmitted] = useState(false);
  const { register, handleSubmit, formState: { errors, isSubmitting }, reset } = useForm<ContactFormData>();

  const onSubmit = async (data: ContactFormData) => {
    try {
      const response = await fetch("/api/contact", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });

      if (response.ok) {
        setSubmitted(true);
        reset();
        setTimeout(() => setSubmitted(false), 5000);
      }
    } catch (error) {
      console.error("Error:", error);
    }
  };

  return (
    <section
      id="contact"
      className="py-12 px-4 sm:px-6 lg:px-8 scroll-mt-20"
      style={{ background: SECTION_COLORS.contact.bg }}
    >
      <div className="max-w-2xl mx-auto">
        {/* Header */}
        <motion.div
          className="text-center mb-8"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          viewport={{ once: true }}
        >
          <h2
            className="text-3xl md:text-4xl font-light mb-6 text-gray-500"
            style={{ fontFamily: "var(--font-poppins)" }}
          >
            Comunicate con nosotros
          </h2>
          <div className="mb-6 space-y-1">
            <p className="text-xl font-bold text-gray-800 flex items-center justify-center gap-2">
              <a
                href={`https://api.whatsapp.com/send?phone=${CONTACT.whatsapp.replace(/[^0-9]/g, "")}`}
                target="_blank"
                rel="noopener noreferrer"
                className="hover:scale-110 transition-transform inline-flex"
                title="Abrir WhatsApp"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#25D366">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
              </a>
              WhatsApp:{" "}
              <a
                href={`https://api.whatsapp.com/send?phone=${CONTACT.whatsapp.replace(/[^0-9]/g, "")}`}
                target="_blank"
                rel="noopener noreferrer"
                className="hover:underline"
                style={{ color: BRAND.colors.primary }}
              >
                {CONTACT.whatsapp}
              </a>
            </p>
            <p className="text-xl font-bold text-gray-800">
              Mail:{" "}
              <a
                href={`mailto:${CONTACT.email}`}
                className="hover:underline"
                style={{ color: BRAND.colors.primary }}
              >
                {CONTACT.email}
              </a>
            </p>
          </div>
          <p className="text-gray-500 text-base">Completá el formulario y te respondemos a la brevedad</p>
        </motion.div>

        {/* Form */}
        <motion.form
          onSubmit={handleSubmit(onSubmit)}
          className="bg-white rounded-2xl shadow-xl p-8 md:p-10"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          viewport={{ once: true }}
        >
          {submitted && (
            <motion.div
              className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg text-green-800 flex items-center gap-3"
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
            >
              <span className="text-2xl">✅</span>
              <div>
                <p className="font-bold">¡Mensaje enviado!</p>
                <p>Nos pondremos en contacto pronto.</p>
              </div>
            </motion.div>
          )}

          {/* Name */}
          <motion.div
            className="mb-6"
            initial={{ opacity: 0, x: -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5, delay: 0.1 }}
            viewport={{ once: true }}
          >
            <label className="block text-sm font-bold mb-3" style={{ color: BRAND.colors.primary }}>
              Nombre
            </label>
            <input
              {...register("name", { required: "El nombre es requerido" })}
              type="text"
              className="w-full px-4 py-3 border-2 rounded-lg focus:outline-none transition"
              style={{ borderColor: errors.name ? "#ef4444" : "#e5e7eb" }}
              placeholder="Tu nombre completo"
            />
            {errors.name && <p className="text-red-600 text-sm mt-2">❌ {errors.name.message}</p>}
          </motion.div>

          {/* Email */}
          <motion.div
            className="mb-6"
            initial={{ opacity: 0, x: -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5, delay: 0.15 }}
            viewport={{ once: true }}
          >
            <label className="block text-sm font-bold mb-3" style={{ color: BRAND.colors.primary }}>
              Email
            </label>
            <input
              {...register("email", {
                required: "El email es requerido",
                pattern: { value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i, message: "Email inválido" },
              })}
              type="email"
              className="w-full px-4 py-3 border-2 rounded-lg focus:outline-none transition"
              style={{ borderColor: errors.email ? "#ef4444" : "#e5e7eb" }}
              placeholder="tu@email.com"
            />
            {errors.email && <p className="text-red-600 text-sm mt-2">❌ {errors.email.message}</p>}
          </motion.div>

          {/* Phone */}
          <motion.div
            className="mb-6"
            initial={{ opacity: 0, x: -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5, delay: 0.18 }}
            viewport={{ once: true }}
          >
            <label className="block text-sm font-bold mb-3" style={{ color: BRAND.colors.primary }}>
              Teléfono
            </label>
            <input
              {...register("phone", { required: "El teléfono es requerido" })}
              type="tel"
              className="w-full px-4 py-3 border-2 rounded-lg focus:outline-none transition"
              style={{ borderColor: errors.phone ? "#ef4444" : "#e5e7eb" }}
              placeholder="Ej: +54 297 4036526"
            />
            {errors.phone && <p className="text-red-600 text-sm mt-2">❌ {errors.phone.message}</p>}
          </motion.div>

          {/* Message */}
          <motion.div
            className="mb-6"
            initial={{ opacity: 0, x: -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5, delay: 0.2 }}
            viewport={{ once: true }}
          >
            <label className="block text-sm font-bold mb-3" style={{ color: BRAND.colors.primary }}>
              Mensaje
            </label>
            <textarea
              {...register("message", { required: "El mensaje es requerido" })}
              className="w-full px-4 py-3 border-2 rounded-lg focus:outline-none transition h-32 resize-none"
              style={{ borderColor: errors.message ? "#ef4444" : "#e5e7eb" }}
              placeholder="Cuéntanos sobre tu proyecto..."
            />
            {errors.message && <p className="text-red-600 text-sm mt-2">❌ {errors.message.message}</p>}
          </motion.div>

          {/* Submit Button */}
          <motion.div className="flex justify-center">
            <motion.button
              type="submit"
              disabled={isSubmitting}
              className="px-8 py-4 rounded-lg font-bold text-white transition shadow-lg"
              style={{
                background: isSubmitting
                  ? "#9ca3af"
                  : `linear-gradient(135deg, ${BRAND.colors.primary} 0%, ${BRAND.colors.secondary} 100%)`,
              }}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              {isSubmitting ? "⏳ Enviando..." : "Enviar Consulta"}
            </motion.button>
          </motion.div>

        </motion.form>
      </div>
    </section>
  );
}

